...
def df_to_github(dataframe, filename, target_folder, repo_name):
    try:
        access_token = read_text_file('authenticator_token.txt')
        g = Github(access_token)
        repo = g.get_repo(repo_name)
        ...
        dataframe.to_excel(excel_data, index=False)
        content_base64 = base64.b64encode(excel_data.getvalue()).decode('utf-8')
        
        # Upload the Excel file to the GitHub repository
        upload_path = f'{target_folder}/{filename}'
        repo.create_file(upload_path, f'uploaded file - {filename}', content_base64, branch='main')
        
        st.success(f"DataFrame uploaded successfully to '{upload_path}'.")
    except Exception as e:
        st.error(f"Error: {e}")
        st.write("DataFrame couldn't be saved to GitHub.")
...
def upload_and_process_data():
    ...
    if uploaded_file:
        ...
        if processed_data is not None:
            ...
            st.code(f'Number of blank rows in columns Opportunity: {blank_opportunity_count} & Opportunity_description : {blank_description_count}')
            ...
            ### Edited for Github
            df_to_github(algo_data,'CGS_Demand_Tracker_Processed.xlsx' , './Processed Files/', repo_name = 'PraneethPonnekanti/acn-cgs-tracker')
            st.info ("Saved processed file to github in the 'Processed Files' folder.")            
...
def retrain_models(inp_data):
    ...
    for classifier_name, classifier in classifiers.items():
        for embedding_name, (X_train, X_test) in embeddings.items():
            ...
            if model_key == best_classifier:
                ...
                #Github specific
                save_model_info_to_github(model_info, classifier_name, embedding_name, repository = 'PraneethPonnekanti/acn-cgs-demand-tracker')
    ...
    save_model_info_to_github(best_model_acc_info, best_model_name_acc.split('_')[0], best_model_name_acc.split('_')[1], repository = 'PraneethPonnekanti/acn-cgs-demand-tracker')
    ...
    upload_to_github('training_output_results.xlsx', '/Processed Files/', repo_name = 'PraneethPonnekanti/acn-cgs-demand-tracker')
...
def predict_using_models(inp_data):
    ...
    if latest_model_info:
        ...
        upload_to_github('cgs_demand_tracker_app_predictions.xlsx', './Output Files/', repo_name = 'PraneethPonnekanti/acn-cgs-demand-tracker')
